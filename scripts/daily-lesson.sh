...
1. List the built English topics (`ls src/content/topics/en/`), then choose ONE at random --
   ...
   run to run; do not always pick the same or the first.
2. Review that topic'\''s current state thoroughly against the mission in CLAUDE.md and its
   skills: zero-to-expert depth, per-concept worked numeric examples, exhaustiveness, animation/
   chart coverage, interaction density, quiz quality (no repeated/interchangeable questions),
   accuracy, accessibility, and en/es twin parity.
