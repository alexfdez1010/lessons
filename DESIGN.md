...
The visual language for **Lessons**: a calm, **blue-forward, light-first**
...
<Callout variant="tip" title="Heads up" client:visible>
  Attention is just a weighted average — the weights are learned.
</Callout>
```
...
<MCQ
  client:visible
  question="Which scales attention scores before softmax?"
  options={[
    { text: '1 / √dₖ', correct: true },
    { text: 'The learning rate' },
    { text: 'Batch size' },
  ]}
  explanation="Scaling by 1/√dₖ keeps the dot products from saturating softmax."
/>
```
...
<Reveal delay={120} client:visible>
  <img src="/diagrams/perceptron.svg" alt="A single perceptron" />
</Reveal>
```
...
Copy-to-clipboard button for commands/snippets. `value` (text to copy),
optional `label`/`copiedLabel`. Async Clipboard API with a legacy fallback;
result announced via `aria-live`.

```mdx
import { CopyButton } from '@/components/react';

<CopyButton client:idle value="bun add @lessons/sdk" />
```
...
is documented in the [README](README.md#seo--open-graph-images).
